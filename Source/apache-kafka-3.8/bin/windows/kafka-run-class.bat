...
rem fix input too long, dikarenakan deklarasi classpath lib java yang masing-masing jar nya didaftarkan, ganti dengan * wildcard
...
IF ["%KAFKA_JMX_OPTS%"] EQU [""] (
	set KAFKA_JMX_OPTS=-Dcom.sun.management.jmxremote=true -Dcom.sun.management.jmxremote.authenticate=false %KAFKA_JMX_ADDITIONAL_OPTS%
)
